...
bool CommandHandler::handleCommands(const std::string& command) {
    ...
    if (command == "clear") {
        ...
        screenS(command.substr(10));
    }
    else if (command.substr(0, 10) == "screen -r ") {
        screenR(command.substr(10));
    }
    else {
...
void CommandHandler::initialize() {
    ...
    std::cout << "Scheduler initialized: "
        << (global_algo == SchedulingAlgorithm::FCFS ? "FCFS" : "RR")
        << ", cores: " << global_core_count << ", quantum: " << global_quantum << std::endl;
    initialized = true;
    printEnter();
}
...
void CommandHandler::schedulerStart() {
    ...
    std::cout << "Scheduler started. Processes will be generated every "
        << global_batch_process_freq << " seconds." << std::endl;
    printEnter();
}
...
void CommandHandler::screenS(const std::string& name) {
    ...
    auto it = std::find_if(processList.begin(), processList.end(), [&](const ProcessInfo& p) {
        return p.name == name;
        });
    ...
    it = std::find_if(processList.begin(), processList.end(), [&](const ProcessInfo& p) {
        return p.name == name;
        });
...
void CommandHandler::screenR(const std::string& name) {
    screenS(name); // Same as screenS but without new process creation
}
...
void CommandHandler::reportUtil() {
    ...
    reportFile << "CPU utilization: " << std::fixed << std::setprecision(2)
    ...
    for (const auto& p : processList) {
        if (!p.finished) {
            ...
            reportFile << "  Core: " << (p.coreID == -1 ? "N/A" : std::to_string(p.coreID));
...
void CommandHandler::printHeader() {
    std::cout << C << " _______  _______  _______  _______  _______  _______  __   __ \n";
    std::cout << C << "|       ||       ||       ||       ||       ||       ||  | |  |\n";
    std::cout << C << "|       ||  _____||   _   ||    _  ||    ___||  _____||  |_|  |\n";
    std::cout << C << "|       || |_____ |  | |  ||   |_| ||   |___ | |_____ |       |\n";
    std::cout << C << "|      _||_____  ||  |_|  ||    ___||    ___||_____  ||_     _|\n";
    std::cout << C << "|     |_  _____| ||       ||   |    |   |___  _____| |  |   |  \n";
